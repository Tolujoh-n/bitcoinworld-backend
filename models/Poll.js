...
const pollSchema = new mongoose.Schema({
  ...
  category: {
    type: String,
    required: true,
    enum: ['Politics', 'Trending', 'Middle East', 'Sports', 'Crypto', 'Tech', 'Culture', 'World', 'Economy', 'Elections', 'Mentions']
  },
  subCategory: {
    type: String,
    required: true,
...
pollSchema.methods.updatePercentages = function() {
  const totalVolume = this.options.reduce((sum, option) => sum + option.totalVolume, 0);
